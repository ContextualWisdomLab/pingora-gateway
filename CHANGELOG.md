...
- Added bounded `PgErdMigrationConfig` and a separate `cwl-pingora-pg-erd-migration` composition root. The operator may configure listener/metrics sockets, non-zero runtime/keepalive budgets, and concrete `backend`/`frontend` transport/TLS values, but may not configure routes, response-policy fields, product auth/business logic, service discovery, or new migration authorities. The generic `GatewayConfig` v1 and `cwl-pingora-gateway` binary remain unchanged.
