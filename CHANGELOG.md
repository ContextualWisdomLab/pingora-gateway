...
- Added `MigrationGatewayProxy` as a pre-listener Pingora callback adapter over the characterized route/header/peer contracts. It rejects unmatched routes, enforces shared body/concurrency isolation, sanitizes untrusted forwarding identity, and applies only the characterized response policy. Production startup still selects the single-upstream `GatewayProxy`; no shadow/canary/cutover is claimed.
