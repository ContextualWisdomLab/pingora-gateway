...
The migration adapter reserves `/livez` and `/readyz` as process-local Pingora health endpoints and does not route them to either consumer origin. The legacy consumer `/healthz` remains distinct routed application traffic to `backend`. Hostile request-controlled `Forwarded`, `X-Forwarded-*`, `X-Real-IP`, and `X-Forwarded-Server` identity is not trusted. `X-Forwarded-For` and `X-Real-IP` are rebuilt from the accepted client socket. For gateway-created `X-Forwarded-Host` and `X-Forwarded-Port`, downstream request-target control data is authoritative when the parsed request URI carries an authority; `Host` is only the origin-form fallback. This prevents a conflicting `Host` field from overriding `:authority`/request-target authority in compatibility metadata while leaving product host authorization outside the gateway. The effective request authority must satisfy RFC 9110/RFC 9113 control-data precedence and RFC 3986 host syntax before it is promoted: userinfo, path/query delimiters, malformed percent escapes, non-IP bracket literals, empty/zero/invalid explicit ports, and unbracketed IPv6 fail closed with HTTP 400. Valid bracketed IPv6/IPvFuture literals and valid `reg-name` syntax remain admissible. `X-Forwarded-Port` uses an explicit port from that effective authority or the admitted scheme default otherwise. The process listener bind port is deliberately not external authority because container, Service, NAT, and port-publish layers may expose a different public port. This local contract governs only the compatibility metadata produced by the migration adapter; Pingora's own outbound authority reconciliation remains supplier behavior and is not reimplemented here. The current captured Traefik entryPoint is cleartext `web`, so this candidate emits downstream scheme `http`. HTTPS/TLS listener behavior requires a separate executable contract.
