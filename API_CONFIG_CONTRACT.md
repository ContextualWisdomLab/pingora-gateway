...
`max_in_flight_requests` is the process-local backpressure boundary for application-shaped downstream requests. The only privileged bypass is an exact payload-free `GET` or `HEAD` to `/livez` or `/readyz` whose downstream body Pingora already reports complete; one valid `Content-Length: 0` remains payload-free only when the transport is likewise complete. Header absence alone is not sufficient: an HTTP/2 request whose HEADERS did not carry `END_STREAM` can still deliver DATA and therefore stays inside ordinary admission. `HEAD` follows RFC 9110 retrieval semantics and returns the same payload-free process-health status without content. A health-path request with `Transfer-Encoding`, duplicate/malformed `Content-Length`, a positive declared length, or an incomplete downstream body first enters ordinary admission and then fails closed with 413; a bodyless method other than GET/HEAD likewise enters ordinary admission and receives 405 with `Allow: GET, HEAD`. When the application budget is exhausted those non-probe shapes therefore fail at admission with 503 rather than acquiring process-health privilege. Every other request remains inside the budget even when the gateway answers locally rather than selecting an upstream, including malformed or exhausted TRACE/OPTIONS `Max-Forwards` requests. The admission lease is released when the request context ends, including failed requests. `upstream_keepalive_pool_size` is wired directly into Pingora's `ServerConf`; the runtime does not inherit Pingora's framework default of 128 reusable upstream connections.
