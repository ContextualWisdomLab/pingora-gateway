...
Unknown fields are rejected. `version` must be `1`. `listener`, `metrics_listener`, and `address` are socket addresses. Traffic and metrics listeners must not overlap one effective socket authority. Equal addresses fail, same-port IPv4/IPv6 wildcard aliases fail, and an IPv6 wildcard plus an IPv4 listener on the same port fails closed because dual-stack bind behavior is platform-dependent. Distinct concrete IP addresses may use the same port. `max_request_body_bytes`, `max_in_flight_requests`, and `upstream_keepalive_pool_size` must all be positive. Generic v1 requires exactly one upstream and a non-empty stable upstream name. Every timeout must be positive.
