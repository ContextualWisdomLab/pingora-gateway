...
//! pinned supplier archive immediately before measurement, extracts a fresh private copy, and
//! executes that freshly derived binary as the terminal command.
...
PG_ERD_GATEWAY_URL=http://127.0.0.1:18180 \
  /tmp/cwl-k6-routed/k6-v2.2.0-linux-amd64/k6 run --quiet tests/load/pg_erd_gateway_smoke.js"#;
...
#[test]
fn live_routed_load_rederives_k6_from_the_pinned_archive_at_measurement_time() {
    let source = fs::read_to_string(CI_WORKFLOW).expect("CI workflow should be readable UTF-8");
    assert!(
        routed_binary_provenance_is_fresh(&source),
        "routed release evidence must revalidate the pinned k6 archive and execute a fresh extraction immediately before measurement"
    );
}
...
#[test]
fn canonical_fresh_extraction_tail_is_admitted() {
    let source = format!(
        r#"
jobs:
  load-contract:
