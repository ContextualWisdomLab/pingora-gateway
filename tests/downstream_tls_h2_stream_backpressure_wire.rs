//! Real-wire HTTP/2 stream-flow-control acceptance for the generic downstream TLS root.
//!
//! A deliberately exhausted response-stream window must backpressure only that stream. The gateway
//! must keep the connection usable for a compliant sibling and resume the stalled response after a
//! stream-scoped WINDOW_UPDATE. Connection-window exhaustion is a separate acceptance increment.
...
const CLIENT_STREAM_WINDOW_BYTES: usize = 16 * 1024;
...
fn issue_gateway_certificate() -> LocalCertificates {
    ...
    run_openssl(&[
        ...
        "-subj",
        "/CN=CWL Downstream H2 Backpressure Test CA",
        "-days",
        "1",
        "-sha256",
    ]);
...
#[test]
fn exhausted_stream_window_backpressures_only_that_stream_and_recovers() {
    ...
    let origin_fixture = thread::spawn(move || {
        ...
        large_writer
            .join()
            .expect("large origin writer should complete");
    });
    ...
    assert_eq!(
        ...
        Some(b"h2".as_slice()),
        "backpressure evidence must run on negotiated h2"
    );
    ...
    for _ in 0..128 {
        ...
        assert!(
            ...
            "stream 1 must stall through flow control rather than reset"
        );
        ...
        if stream_id == 1 && frame_type == H2_FRAME_DATA {
            ...
            assert!(
                ...
                "server exceeded the client-advertised stream window without stream credit"
            );
            if stream1_body.len() == CLIENT_STREAM_WINDOW_BYTES {
                break;
            }
        }
    }
    ...
    assert_eq!(
        ...
        "stream 1 must consume exactly the advertised stream window before stalling"
    );
    ...
    tls.flush()
        .expect("sibling request should flush while stream 1 is flow-controlled");
    ...
    for _ in 0..128 {
        ...
        assert_ne!(
            frame_type, H2_FRAME_GOAWAY,
            "stream-level backpressure must not poison the H2 connection"
        );
        assert!(
            frame_type != H2_FRAME_RST_STREAM || stream_id != 3,
            "compliant sibling must not be reset"
        );
        if handle_control_frame(&mut tls, frame_type, flags, stream_id, &payload) {
            continue;
        }
        if stream_id == 1 && frame_type == H2_FRAME_DATA {
            assert!(
                payload.is_empty(),
                "stream 1 must not receive non-empty DATA before stream-scoped WINDOW_UPDATE"
            );
        }
        ...
        if stream_id == 3 && frame_type == H2_FRAME_DATA {
            ...
            if !payload.is_empty() {
                write_window_update(&mut tls, 0, payload.len());
                tls.flush()
                    .expect("sibling connection WINDOW_UPDATE should flush");
            }
        }
    ...
    assert!(
        sibling_ended,
        "sibling must complete while stream 1 remains flow-controlled"
    );
    assert_eq!(sibling_body, b"sibling-ok");

    let remaining = LARGE_BODY_BYTES - CLIENT_STREAM_WINDOW_BYTES;
    write_window_update(&mut tls, 1, remaining);
    tls.flush()
        .expect("stream 1 WINDOW_UPDATE should release the stalled response");
    ...
    for _ in 0..256 {
        ...
        assert_ne!(
            frame_type, H2_FRAME_GOAWAY,
            "connection must remain live while the stalled stream resumes"
        );
        assert!(
            frame_type != H2_FRAME_RST_STREAM || stream_id != 1,
            "resumed stream 1 must not be reset"
        );
        if handle_control_frame(&mut tls, frame_type, flags, stream_id, &payload) {
            continue;
        }
        if stream_id == 1 && frame_type == H2_FRAME_DATA {
            ...
            if !payload.is_empty() {
                write_window_update(&mut tls, 0, payload.len());
                tls.flush()
                    .expect("resumed connection WINDOW_UPDATE should flush");
            }
        }
    ...
    assert!(
        ...
        "flow-controlled response bytes must remain intact across stall/resume"
    );

    origin_fixture
        .join()
        .expect("flow-control origin fixture should complete");
}
