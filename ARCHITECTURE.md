...
The **Observability** bounded context owns low-cardinality transport completion facts and shared gateway counters/access-log shape. `RequestObservation` contains only downstream status, `ok`/`error`, and observed request-body bytes. The cutover discriminator `cwl_pingora_gateway_requests_by_transport_total` adds only `outcome={ok,error}`, `protocol={h1,h2}`, and `transport={cleartext,tls}`, for a fixed eight-series Cartesian product derived from the accepted Pingora session and transport digest. Paths, query strings, headers, cookies, credentials, client addresses, certificate paths, deployment revisions, customer payloads, private-key material and product identifiers remain outside the shared telemetry contract.
...
The current TLS increment admits HTTP/2 only through TLS ALPN and explicitly allows HTTP/1.1 fallback. It does not admit h2c. The current writer-safe stack now has executable real-wire evidence for concurrent H2 streams, stream reset/cancellation and sibling survival, GOAWAY/drain, decoded-header and request-body admission, stream- and connection-window backpressure/recovery, bounded upstream read-ahead under exhausted connection credit, partial-body cleanup, pre/post-commit origin reset recovery, TLS-derived `Forwarded` scheme, and bounded H2/TLS cutover transport telemetry. Those are transport/runtime acceptance slices, not release parity. H2-downstream to H1-upstream Cookie and zero-length body-framing correctness remain gated by release-qualified supplier disposition; representative TLS handshake versus reused-connection timing, many-core/NUMA performance, immutable release identity, concrete parity/shadow/canary, observed rollback, cutover and verified legacy removal remain separate promotion evidence.
