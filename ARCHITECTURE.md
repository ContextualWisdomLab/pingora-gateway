...
The future **Downstream TLS / Protocol Admission** bounded context is reserved by issue #51 and Proposed ADR 0012 for a versioned listener-side transition beyond the current clear-text v1 contracts. It will own only the reusable invariant that decides whether a process may terminate TLS and admit HTTP/1.1 and/or HTTP/2 for an explicitly characterized listener. Certificate/key references, SNI mapping and ALPN policy are listener-transport values; certificate issuance/renewal/ACME/private-key custody, product authentication and Keyverse identity remain with their canonical owners. The context is not active in generic v1 or the current pg-erd profile: both composition roots still bind application traffic with Pingora `add_tcp`, while all current upstream peers remain explicitly `ALPN::H1`. Consequently, downstream H2 would immediately exercise H2-to-H1 translation and cannot be credited until the exact supplier passes the #51 mixed-protocol acceptance, including `cloudflare/pingora#935/#936` and #892 characterization. HTTP/3/QUIC remains outside this boundary until a separately accepted immutable transport capability exists.
...
The **Runtime Isolation** bounded context owns transport-neutral request-body and concurrent-request admission budgets shared by both Pingora adapters. For the version-2 bounded pg-erd profile it also owns the opt-in monotonic upstream response-body progress lifetime: the budget starts at the first non-informational upstream response header and is checked on body-progress callbacks without resetting on progress. This is deliberately not an exact interrupt for a pending supplier read. The generic v1 runtime has no whole-response lifetime. Supplier-owned protocol/runtime limits that cannot be implemented before Pingora callbacks remain explicit external dependencies rather than copied into this bounded context: #43/#993 is downstream H1 parser byte/count admission, #45/#447 is downstream H1 whole-request-header lifetime, #40/#992 is upstream response-header lifetime, and #46/#844/#969 is parked-read graceful-shutdown correctness plus shared-`Notify` scaling. The last path is distinct from the already-admitted in-flight request drain proved by PR #20. An open contributor PR is not a Shared Kernel or released contract; a maintainer-integrated immutable release or separately governed provenance-bound backport is required before Runtime Isolation may consume that capability.
...
future characterized TLS/H2 consumer evidence
                |
                v
  Downstream TLS / Protocol Admission --(versioned listener values)--> Pingora TLS/H2 listener
                |
                +-- certificate/key references only --> canonical certificate/secret owner
...
`edge_contract`, `edge_routing`, `http_policy`, `migration_plan`, `migration_admin`, `forwarding_policy`, `runtime_isolation`, and the transport-only observation vocabulary are anti-corruption boundaries around Pingora delivery semantics. Proposed Downstream TLS / Protocol Admission is likewise a transport boundary, not a certificate lifecycle, product identity, authorization or upstream-protocol owner. `migration_delivery` and `pingora_delivery` translate already admitted network values to `HttpPeer`; `gateway_proxy` and `migration_proxy` compose those contracts through `ProxyHttp`. Reversing these dependencies, importing product-domain authorization/business code, trusting request-controlled proxy identity, performing implicit service discovery, recording request/customer payloads in shared telemetry, copying mutable supplier lifecycle/parser/protocol internals into CWL ownership, or letting request-controlled destinations become upstream authority is a DDD defect.