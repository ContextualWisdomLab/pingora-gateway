...
The future **Downstream TLS / Protocol Admission** bounded context is reserved by issue #51 and Proposed ADR 0012. It will own only the reusable listener-side invariant that decides whether an explicitly characterized process may terminate TLS and admit HTTP/1.1 and/or HTTP/2. Certificate/key references, SNI mapping and ALPN policy are transport configuration values; certificate issuance/renewal/ACME/private-key custody, Keyverse identity, product authentication/business policy, and upstream protocol authority remain outside this context. Generic v1 and the current pg-erd profile do not activate it: both production composition roots use Pingora `add_tcp`, and every characterized upstream peer remains `ALPN::H1`. Downstream H2 therefore cannot be credited until exact-supplier H2-to-H1 translation traffic closes the supplier paths tracked by `cloudflare/pingora#935/#936` and #892. HTTP/3/QUIC remains a separate unsupported transport contract.
