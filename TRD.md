...
Graceful shutdown is a distinct supplier-bound parked-read requirement. The current #44/#47 parent still consumes affected Pingora 0.8.0 exact `09696b51bc59315353d96686355861604d0bb48c`; historical source and #844 characterize both a one-shot lost-wakeup window and shared-`Notify` many-core contention. Supplier revalidation on 2026-09-20 changes the required action: Pingora 0.9.0 exact tag `702f69015e53f7244d6ad2e743de571d859a70a4` publicly contains `ShardedNotify`, derives shard count from configured `ServerConf::threads`, registers the shutdown waiter before consulting the shutdown flag, and notifies all shards. #844 is closed as completed and historical contributor PR #969 is closed without direct GitHub merge. CWL therefore must adopt and validate the released supplier behavior rather than pinning the contributor branch or copying `pingora-proxy`. #70 owns the 0.9.0 consumer transition, #73 explicit worker topology, and #74 representative Linux/NUMA profiling; each must reacquire evidence after parent movement.
...
Supply-chain evidence remains exact-source-bound: committed lockfile, dependency/advisory policy, SBOM, both candidate image builds, and image vulnerability scans. These are candidate receipts, not immutable release identity. Protected promotion additionally requires immutable package/image digests, release-bound SBOM/provenance/attestation, reproducibility, rollback rehearsal, and current protected ancestry. Pingora 0.9.0 is the public shutdown-repair identity, but #70 must consume it on current ancestry and #73/#74 must supply current topology/scaling evidence; independent security blockers, including the supplier `derivative 2.2.0 / RUSTSEC-2024-0388` root, remain fail closed.
...
Issue #46 adds a distinct hot-runtime performance requirement after released-supplier adoption: parked HTTP/1 registration/unregistration and shutdown must be profiled with the actual configured proxy worker topology in the representative Linux/NUMA lane. #74 retains documented floors for worker count, parked reusable connections, shutdown rounds and close bound, and requires complete stable-TID scheduler evidence; lowering pressure merely to avoid contention is not equivalent evidence.
