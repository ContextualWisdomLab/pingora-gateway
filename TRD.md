...
The admitted downstream ALPN policy is `h2_http1`. The delivery adapter uses Pingora/OpenSSL's public ALPN callback surface to prefer `h2`, fall back to `http/1.1` only when the client offers it, and return a fatal handshake error for an ALPN-bearing client with no admitted protocol overlap or a malformed protocol vector. This is intentionally stricter than Pingora 0.9.0 `TlsSettings::enable_h2()`, whose `H2H1` no-overlap branch returns `NOACK`; RFC 7301 requires fatal `no_application_protocol` on no overlap. TLS-enabled roots use `add_tls_with_settings`; cleartext versions continue to use `add_tcp`. h2c is not enabled and must not be inferred from HTTP/2 support.

Real-wire acceptance must verify the configured server identity against a local CA, assert negotiated ALPN and pass an actual request through the production composition path. HTTP/1.1 fallback must remain deliberate and separately exercised. A client that sends ALPN but offers only unsupported protocols must fail the TLS handshake. Invalid, unreadable or mismatched certificate/key material must stop startup. Test child processes must be bounded and reaped on failure, timeout and panic paths.

Basic H2 wire traffic is not complete H2 operational parity. The current writer-safe stack has executable real-wire acceptance for concurrent streams, reset/cancellation and sibling survival, GOAWAY/drain, decoded-header and request-body admission, stream- and connection-window backpressure/recovery, bounded upstream read-ahead under exhausted connection credit, partial-body upstream cleanup, pre/post-commit origin failure/recovery, TLS-derived generic `Forwarded` scheme, and bounded H2/TLS cutover transport telemetry. These slices do not replace release-qualified H2-downstream to H1-upstream Cookie coalescing/body framing, representative TLS handshake versus reused-connection performance, representative CPU/NUMA profiling, immutable release identity, concrete parity/shadow/canary, observed rollback, cutover or verified legacy removal. Mutable supplier implementations must not be vendored to manufacture parity.
...
Shared telemetry is low-cardinality and payload-free. Aggregate request/error/body-byte/backpressure facts remain backward-compatible. The cutover metric `cwl_pingora_gateway_requests_by_transport_total` adds only the bounded labels `outcome={ok,error}`, `protocol={h1,h2}`, and `transport={cleartext,tls}`; the eight possible series are derived from the accepted Pingora session and transport digest rather than request-controlled headers. Authorization, cookies, tokens, arbitrary paths, hostnames, client addresses, certificate paths, deployment revisions, customer/product identifiers, payloads, trust-bundle contents, certificate private-key material and configuration credentials must not enter shared logs or metric labels. Pingora-family dependency logs remain subject to the repository logging policy.
...
Consumer migration requires a real immutable gateway identity, deployment pin, parity validation on the concrete edge, shadow/canary, observed rollback, cutover and verified legacy Nginx/OpenResty removal. The presence of downstream TLS/H2 source does not skip supplier framing roots or the remaining issue #51 protocol/performance/promotion matrix.
