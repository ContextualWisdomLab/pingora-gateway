...
The exact-head `load-contract` lane builds both release binaries and keeps the generic and pg-erd load contracts distinct while sharing one bounded std-only Rust HTTP/1.1 origin implementation. CI format-checks `tests/load/load_origin.rs`, executes its direct startup/framing parser tests with `rustc --test`, and builds the helper with optimization before measured traffic. The fixture validates startup controls before bind, caps workers at 256, bounds accepted-but-unassigned sockets and request-header buffering, uses deterministic `Content-Length` framing and `TCP_NODELAY`, and is test tooling rather than a production target. `tests/rust_load_origin_workflow_contract.rs` parses only the `load-contract` job and fails if Python appears anywhere in that measured job or if generic/backend/frontend origins stop using the bounded Rust fixture.

The generic contract remains a minimal one-upstream loopback regression. The pg-erd contract starts distinct `backend` and `frontend` instances of the same Rust fixture through startup-only `UPSTREAM_PORT`/`UPSTREAM_PAYLOAD` values, configures the release-mode `cwl-pingora-pg-erd-migration` binary with explicit non-zero in-flight, keepalive, and I/O budgets, and performs only origin `/fixture-ready` plus process `/livez` probes before measurement; measured application routes are not warmed.

`tests/load/pg_erd_gateway_smoke.js` sends 400 total requests across four VUs, alternating `/api/load-contract` to the characterized `backend` and `/load-contract` to `frontend` independently inside each VU. Each measured request is tagged with its characterized route identity. Both route-tagged `http_reqs` counters must contain at least 198 samples, so a route-local percentile cannot pass on an accidentally tiny subset of the pooled traffic. Both route-specific bodies must remain correct, every k6 check must pass, `http_req_failed` must remain zero, and `http_req_duration` p95 must stay below 20 ms both for the aggregate sample and independently for `route:backend` and `route:frontend`. `tests/pg_erd_routed_latency_contract.rs` locks the alternating route/path/body mapping, route tags, route-local sample floors, and per-route latency thresholds. A successful lane requires `k6-pg-erd-summary.json`; the always-run artifact upload tolerates a missing summary only after an earlier causal failure so evidence handling does not replace that failure with a secondary upload error. The generic summary follows the same success-versus-diagnostics rule.

This p95 threshold is a deterministic Rust-only local regression bound, not a production latency claim. It does not establish TLS handshake cost, Internet or multi-hop network behavior, representative origin capacity, Kubernetes scheduling, shadow/canary traffic, or buyer deployment SLOs. Production performance credit requires representative deployment measurement on an immutable release identity without sample reduction, measurement exclusion, or application-route cache warm-up.
