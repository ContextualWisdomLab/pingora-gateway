...
PR #70 turns the correctness gap into compiled consumer traffic without changing production gateway Rust or the supplier pin. The generic v1 and bounded pg-erd fixtures preserve the existing admitted-work control, while `tests/shutdown_post_notify_barrier.rs` adds a causal external ordering proof that listener closure plus a fixed sleep alone cannot provide.

For each composition root, one subject request is admitted and held at its origin. A separate sentinel connection first completes gateway-local `/livez` over HTTP/1.1 keep-alive and sends a partial next request. After a short settle interval, one bounded pre-SIGTERM read must return only `WouldBlock` or `TimedOut`; EOF or response bytes fail the fixture. This proves only the externally observable condition needed by the oracle: immediately before SIGTERM the incomplete sentinel connection is still open and response-free. It does not claim direct observation of Pingora's internal `Notified` registration. After SIGTERM stops public accepts, that same sentinel must receive EOF inside a one-second window that remains strictly before the five-second runtime fallback. The transition from pre-signal open/no-response to post-signal EOF is the external cleanup barrier. Only after that barrier does the fixture release the held `Connection: keep-alive` response on the subject connection. The subject must complete its admitted body and then receive EOF on its newly created next keep-alive read inside the same pre-fallback bound. Pinned public Pingora is expected to fail that final post-notification close because `notify_waiters()` stores no permit and `handle_new_request()` does not consult `shutdown_flag` before parking.

The origin response-release channel has its own 30-second watchdog, using the already-governed external termination budget, solely to bound fixture leakage or a dead controller. It is not a second graceful-shutdown clock: it begins before SIGTERM, while the product acceptance remains anchored to `signal_sent_at`, the one-second EOF evidence window, and the unchanged five-second production grace. Keeping those clocks separate prevents a slow hosted runner before signal delivery from manufacturing a false RED.

The sentinel changes false-GREEN direction intentionally. A sentinel that was already closed or had received response bytes before SIGTERM is now rejected by the bounded pre-signal read, so later EOF cannot be misattributed to shutdown cleanup. The fixture still does not inspect Pingora's internal waiter state; it uses the observable incomplete-request connection state plus the post-signal EOF transition as its barrier. Origin/header evidence remains finite at five seconds and 64 KiB, and HTTP success requires the exact `HTTP/1.1 200 ` status token so numeric-prefix lookalikes cannot manufacture GREEN.

## RED / GREEN evidence contract

RED must exercise the exact compiled generic and bounded pg-erd composition roots. Preserve an admitted request across SIGTERM, externally prove that the incomplete sentinel is open and response-free immediately before SIGTERM and then closed inside the bounded post-signal window, then release the admitted keep-alive response and demonstrate that the affected supplier leaves the subject's newly created post-notification read parked until the bounded evidence window expires. Separately run repeated scheduling jitter around shutdown and characterize high-concurrency registration/unregistration at realistic configured worker counts, capturing shutdown wall time, CPU utilization, and Linux off-CPU/futex evidence where the environment permits it. Do not manufacture RED by shrinking the production grace period, disabling keep-alive, reducing worker topology, or weakening the close oracle.
