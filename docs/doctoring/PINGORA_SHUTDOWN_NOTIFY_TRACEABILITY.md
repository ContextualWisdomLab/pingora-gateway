...
PR #70 turns the correctness gap into compiled consumer traffic without changing production gateway Rust or the supplier pin. Both the generic v1 process and the bounded pg-erd composition root preserve an already-admitted request, send SIGTERM only after origin admission, wait until the public traffic listener has stopped accepting, then release a `Connection: keep-alive` response while still inside the production grace period. The same downstream connection must complete that admitted response and then observe EOF on its next keep-alive read within a one-second evidence bound, before runtime fallback can begin. Origin request acquisition has a five-second socket bound, fixture evidence is capped at 64 KiB, and HTTP success requires the exact `HTTP/1.1 200 ` status token so numeric-prefix lookalikes cannot manufacture GREEN.

This deterministic post-notification park is the minimal causal correctness RED. Repeated jitter and high concurrency remain required before commercial closure because they characterize adjacent scheduling states and the performance side of #46; they are not used to make the one-shot semantics ambiguous.

## RED / GREEN evidence contract

RED must exercise the exact compiled generic and bounded pg-erd composition roots. Preserve an admitted request across SIGTERM, then force its reused keep-alive connection to begin the next `read_request()` after cleanup's one-shot `notify_waiters()` has already fired and demonstrate that the affected supplier leaves that read parked until the bounded evidence window expires. Separately run repeated scheduling jitter around shutdown and characterize high-concurrency registration/unregistration at realistic configured worker counts, capturing shutdown wall time, CPU utilization, and Linux off-CPU/futex evidence where the environment permits it. Do not manufacture RED by shrinking the production grace period, disabling keep-alive, reducing worker topology, or weakening the close oracle.

GREEN requires a supported supplier capability on an immutable/released or explicitly governed backport identity. Re-run the deterministic post-notification assertion, repeated jitter and high concurrency; require zero parked-read survivors beyond the bounded shutdown expectation, retain successful completion of already-admitted in-flight requests inside the configured grace, and preserve `/livez`/`/readyz` behavior. If the supplier repair shards waiters, verify the shard count against actual configured worker/runtime topology rather than assuming host `available_parallelism()` is equivalent. No request path, header, cookie, credential, or customer payload is needed in this evidence.
