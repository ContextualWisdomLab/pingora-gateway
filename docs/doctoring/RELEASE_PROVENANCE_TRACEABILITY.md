...
Exact `7517c3691be1b4437914bb7498f95f105bf98d88` adds `tests/release_provenance_workflow_contract.rs` before changing the workflow. The contract requires OIDC/attestation permissions, one immutable `actions/attest` invocation, both release binaries as subjects, post-attestation verification, exact signer-workflow identity, exact source and signer digest binding, and hosted-runner verification. The parent #90 workflow lacks those controls, so the test is an executable RED specification rather than predecessor evidence reuse.

The minimal repair extends the existing reproducibility workflow after successful byte comparison. It attests both exact binaries and immediately verifies each artifact with `gh attestation verify`, binding the repository, signer workflow `ContextualWisdomLab/pingora-gateway/.github/workflows/release-reproducibility.yml`, source digest, signer digest, and GitHub-hosted runner requirement to the current `EXPECTED_SHA`.
...
5. `gh attestation verify` succeeds separately for both binaries while enforcing this repository, this exact signer workflow, `source-digest=$EXPECTED_SHA`, `signer-digest=$EXPECTED_SHA`, and denial of self-hosted-runner provenance;
6. normal CI, Supply Chain, capacity, TLS/H2 performance, reproducibility, review-thread, and governance evidence remain exact-current.

## Risk and interpretation

A GREEN result authenticates the exact candidate binaries to a GitHub Actions workflow identity and exact source/signer digest using GitHub's OIDC/Sigstore attestation system. GitHub CLI documentation notes that certificate identity and verified timestamps are the portions not controlled by workflow-authored predicate content; the selected verification therefore binds actor identity rather than trusting predicate fields alone.
