...
- Source identity and rendered-content identity are separate claims. A public `/source-sha.txt` can prove that one file from the new deployment is visible while an intermediary cache or partial publication still serves an older `/` document. The build therefore writes `index-sha256.txt` from the exact rendered `_site/index.html` before upload. Post-deploy verification fetches the source marker, that digest file, and the site root directly from the deployment URL, rejects redirects and non-200 responses for all three, validates the digest syntax, and requires SHA-256 of the fetched root bytes to equal the digest carried by the uploaded artifact. This prevents a fresh marker plus stale buyer-facing root from satisfying publication evidence.
...
- digest syntax validation and exact SHA-256 equality between the fetched root bytes and the digest generated before artifact upload; and
