...
PR #79 is the H2 graceful-shutdown child. Current exact `9b092c46a03dcbf59c48718f519fa2285f27bf37` repairs an inverted shutdown oracle after re-reading released Pingora 0.9.0 source. Pingora broadcasts the service shutdown watch before the process-level grace sleep, so the initial `GOAWAY(NO_ERROR, 2^31-1)` is required to appear after SIGTERM while the configured five-second runtime grace interval is still active, not after that interval. Stream 3 is sent after SIGTERM but before the client observes GOAWAY and may race into origin during the same interval; the oracle no longer requires origin admission to precede client observation of initial GOAWAY. Both admitted/racing streams must still complete without reset, later GOAWAY last-stream-id must never increase, and final drain evidence remains bounded. `src/runtime_policy.rs` now states the same supplier ordering. Exact-current PgErd bounded-origin capacity `34571125411` is GREEN; CI `34571125482` and Supply Chain `34571125427` remain current-head promotion authority until terminal.

PR #80 is the decoded-header admission child. Current exact `e7613241e2da98cb7a71281ca343e8d2017e3fd9` is an ordinary two-parent, non-force restack that adopts #79 `9b092c46...` while preserving the effective child delta as only `tests/downstream_tls_h2_header_admission_wire.rs`. The fixture requires `SETTINGS_MAX_HEADER_LIST_SIZE=65536`, sends a legal multi-frame HPACK field section whose decoded size exceeds that advertised bound, requires rejection before origin contact without connection-level GOAWAY, and then requires a compliant sibling on the same H2 connection to reach origin and complete with exact body `sibling-ok`. It intentionally does not independently HPACK-decode the rejection response and therefore does not claim exact 431 wire status. Exact-current CI `34571540407`, Supply Chain `34571540788`, and capacity `34571540404` must terminate before promotion.

PR #81 is the request-body transport-parity child, stacked directly on current #80. It adds negotiated-H2 evidence for both body-admission paths under an 8-byte limit. Stream 1 presents `content-length: 9` without DATA and must receive a stream-local HTTP response; stream 3 sends an undeclared nine-byte DATA body and must also receive an HTTP-level stream-local response. A compliant stream 5 must then reach the H1 origin on the same H2 connection and return exact body `sibling-ok`. The origin fixture permits streamed-request header contact because incremental enforcement may run after upstream headers; it does not reclassify that transport detail as a policy defect. The fixture does not independently decode the rejection status, so exact H2 413 remains a combined production-adapter/HTTP1-wire contract rather than a new claim from this test alone. The branch has been non-force restacked on #80 current; #81 remains Draft until its own exact formatting, compile/test, coverage, supply-chain and capacity evidence is terminal.
...
Cloudflare, Inc. (2026). *Pingora server runtime and graceful-shutdown implementation* (source `702f69015e53f7244d6ad2e743de571d859a70a4`). https://github.com/cloudflare/pingora/blob/702f69015e53f7244d6ad2e743de571d859a70a4/pingora-core/src/server/mod.rs
