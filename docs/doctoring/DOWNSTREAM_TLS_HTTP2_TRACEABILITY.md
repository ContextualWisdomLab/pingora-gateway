...
Released dependency authority remains `pingora = 0.9.0` and `pingora-prometheus = 0.9.0` from the registry with the committed Cargo lock. Pingora tag `0.9.0` resolves exactly to `702f69015e53f7244d6ad2e743de571d859a70a4`. The resolved HTTP/2 codec is `h2 = 0.4.19`; tag `v0.4.19` resolves to `d57d1b852fec9dda6d42d3454502006d52104da8`.

At the released Pingora source, `TlsSettings::intermediate(cert_path, key_path)` constructs the server acceptor and loads PEM material, while CWL performs an explicit private-key/certificate pairing check before listener activation. Pingora exposes its OpenSSL-compatible acceptor builder, allowing CWL to install the stricter ALPN callback without copying supplier source. `add_tls_with_settings` is used only after versioned transport-neutral configuration admission and materialization.
...
Pingora 0.9.0 graceful shutdown has two process phases that matter to H2 evidence. Its Unix `main_loop` broadcasts `shutdown_watch = true` to services before returning `ShutdownType::Graceful`; `Server::run` only then enters the configured `grace_period_seconds` sleep before runtime shutdown. HTTP/2 services can therefore start graceful GOAWAY processing while the process-level grace interval is still active. CWL must not reinterpret that interval as a mandatory delay before GOAWAY emission.

The resolved h2 0.4.19 implementation adds another wire-level requirement. Server `go_away_gracefully()` sends initial `GOAWAY(NO_ERROR, 2^31-1)` and calls `ping_shutdown()` because the implementation waits one RTT before proceeding to its later GOAWAY. A raw H2 acceptance client must therefore ACK the non-ACK PING with the same eight-octet payload. Ignoring that PING is a client-fixture defect: it can stall the graceful sequence until the surrounding runtime closes the socket and manufacture an `UnexpectedEof` while waiting for later drain evidence.
...
PR #79 is the H2 graceful-shutdown child. Released-source review first repaired an inverted shutdown oracle: Pingora broadcasts service shutdown before the process-level grace sleep, so initial `GOAWAY(NO_ERROR, 2^31-1)` belongs after SIGTERM while the five-second runtime grace interval is active rather than after that interval. Exact `9b092c46a03dcbf59c48718f519fa2285f27bf37` then produced a focused CI RED: formatting, OCI runtime, load contract, and all unrelated tests passed, while `downstream_tls_h2_goaway_drain_wire` ended with `UnexpectedEof` waiting for a later H2 frame. The raw fixture ACKed SETTINGS but not the h2 0.4.19 graceful-shutdown PING. Current exact `fa064688187e15ae16d48ee227b09f9a150e5a6e` minimally repairs the client by validating the eight-octet PING payload and echoing it in `PING ACK` in both drain read loops. Production Rust, shutdown timing, routing, and HTTP policy are unchanged. Fresh exact-current runs are CI `34572255720`, Supply Chain `34572255776`, and PgErd bounded-origin capacity `34572256114`; predecessor receipts do not transfer.

PR #80 is the decoded-header admission child. Current exact `b989145fc2a4967468467c0226b38c9ced278e27` is an ordinary two-parent, non-force restack that adopts #79 `fa064688...` while preserving the effective child delta as only `tests/downstream_tls_h2_header_admission_wire.rs`. Fresh comparison has merge base exactly #79 current and behind count zero. The fixture requires `SETTINGS_MAX_HEADER_LIST_SIZE=65536`, sends a legal multi-frame HPACK field section whose decoded size exceeds that advertised bound, requires rejection before origin contact without connection-level GOAWAY, and then requires a compliant sibling on the same H2 connection to reach origin and complete with exact body `sibling-ok`. It intentionally does not independently HPACK-decode the rejection response and therefore does not claim exact 431 wire status. Fresh exact-current runs are CI `34572379517`, Supply Chain `34572379510`, and capacity `34572379545`; predecessor receipts do not transfer.

PR #81 is the request-body transport-parity child stacked on #80 current. It adds negotiated-H2 evidence for both body-admission paths under an 8-byte limit. Stream 1 presents `content-length: 9` without DATA and must receive a stream-local HTTP response; stream 3 sends an undeclared nine-byte DATA body and must also receive an HTTP-level stream-local response. A compliant stream 5 must then reach the H1 origin on the same H2 connection and return exact body `sibling-ok`. The origin fixture permits streamed-request header contact because incremental enforcement may run after upstream headers; it does not reclassify that transport detail as a policy defect. The fixture does not independently decode the rejection status, so exact H2 413 remains a combined production-adapter/HTTP1-wire contract rather than a new claim from this test alone. The branch has been non-force restacked on #80 current and this traceability note was ordinary-forward repaired for the #79 PING-ACK RCA/current stack. #81 remains Draft until its own exact formatting, compile/test, coverage, supply-chain, and capacity evidence is terminal.
...
Cloudflare, Inc. (2026). *Pingora server configuration* (release 0.9.0, source `702f69015e53f7244d6ad2e743de571d859a70a4`). https://github.com/cloudflare/pingora/blob/702f69015e53f7244d6ad2e743de571d859a70a4/pingora-core/src/server/configuration/mod.rs
...
Cloudflare, Inc. (2026). *Pingora TLS listener implementation* (release 0.9.0, source `702f69015e53f7244d6ad2e743de571d859a70a4`). https://github.com/cloudflare/pingora/blob/702f69015e53f7244d6ad2e743de571d859a70a4/pingora-core/src/listeners/tls/boringssl_openssl/mod.rs
...
Hyperium. (2026). *h2 graceful GOAWAY implementation* (release 0.4.19, source `d57d1b852fec9dda6d42d3454502006d52104da8`). https://github.com/hyperium/h2/blob/d57d1b852fec9dda6d42d3454502006d52104da8/src/proto/connection.rs
