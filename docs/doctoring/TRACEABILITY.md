...
| Bash command substitution supports both `$(command)` and the legacy backquote form; the command executes in a subshell environment, while in the legacy form the first unescaped backquote terminates the substitution | GNU Bash Reference Manual, *Command Substitution*, revalidated 2026-09-06. Release-path shell control already rejects active legacy backquotes; the narrower command-substitution companion now applies the existing compiler-authority analyzer to active backquote bodies instead of silently treating them as plain text |
