...
| Claim | Source |
| --- | --- |
| Pingora server/proxy composition and graceful server lifecycle | Cloudflare Pingora source at pinned commit `09696b51bc59315353d96686355861604d0bb48c`, still the protected upstream `main` head observed on 2026-09-05 |
...
| GNU Coreutils `env -S` / `--split-string=STRING` performs a second argument-splitting pass; short options can be combined (for example `-vS`); normal `env` options must precede operands; `NAME=VALUE` operands precede the first non-assignment command operand and all later words are child-command arguments | GNU Coreutils 9.11 manual, *env invocation*; the gateway release-path oracle therefore fails closed on split-string/unmodelled option grammar, consumes documented argument-taking options, inspects compiler assignments only before the child command, and does not misclassify child arguments such as `/usr/bin/printf -S` as `env` options |
