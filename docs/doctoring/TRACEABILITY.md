...
| Supplier PR #936 proposes the narrow BodyWriter repair: zero-length chunked writes become no-ops in both async and cancel-safe task paths and `finish()` remains the sole terminator emitter; it is not yet an immutable maintainer-integrated dependency | Cloudflare Pingora PR #936, head `e40ed4cceb0c0ed8c05cc39eb01a8c73dea5497a`, exactly one commit whose parent is protected public `main@09696b51bc59315353d96686355861604d0bb48c`, one changed file (`pingora-core/src/protocols/http/v1/body.rs`), open/mergeable and unmerged when revalidated 2026-09-04. The contributor retained both async and cancel-safe regression tests, reported `cargo test -p pingora-core --lib` passing 544 tests, and exact-head supplier Semgrep/build matrix runs are terminal success; formal review count remained zero on revalidation, so downstream mutable-branch pinning remains prohibited until a maintainer-integrated immutable dependency identity exists |
| H2-to-H1 Cookie translation on the protected supplier still lacks the RFC-required coalescing step: multiple decompressed H2 `Cookie` fields must become one H1 field joined with `"; "` before entering the non-H2 context | RFC 9113 §8.2.3; protected Pingora `main@09696b51bc59315353d96686355861604d0bb48c` `pingora-proxy/src/proxy_h1.rs::proxy_1to1()` converts the request to H1, calls `sanitize_h1_upstream_request()`, finalizes framing and writes the header, while exact `proxy_common.rs::sanitize_h1_upstream_request()` handles Connection nominations, hop-by-hop fields and Upgrade policy but contains no Cookie concatenation. Cloudflare issue #892 is the defect authority; collaborator comment 4579330810 supports default-on normalization with an explicit opt-out. Public implementation candidate #901 exists at contributor head `b856ddfc6be15f1727601d2d76cb10d2d72f95f0`, but fresh comparison against protected main is 2 commits ahead / 125 behind with merge base `c0845a8693b0792a6ccd0626e8475990f7269af2`, both touched files have intervening mainline changes, and the PR is open/unmerged/mergeable=false with no formal review. Its historical Semgrep run succeeded while Rust 1.91.1 and nightly build jobs failed during `cargo test`; the expired historical logs do not support a trustworthy RCA. Current protected main has since centralized request forwarding in `HttpUpstreamRequestPolicy` plus `sanitize_h1_upstream_request()`, so #901 requires owner-side current-main restack/adaptation. The Cookie opt-out must remain narrow rather than requiring broad `HttpUpstreamRequestPolicy::preserve()`, which also disables unrelated forwarding protections. Downstream consumers may use only a maintainer-integrated immutable supplier identity and must rerun the unchanged real-wire contract |
...
Cloudflare. (n.d.). *Pingora HTTP server options* [Source code, commit 09696b51bc59315353d96686355861604d0bb48c]. GitHub. https://github.com/cloudflare/pingora/blob/09696b51bc59315353d96686355861604d0bb48c/pingora-core/src/apps/mod.rs
...
MyLittleLuckyDog. (2026, June 3). *fix(proxy): concatenate HTTP/2 Cookie headers when proxying to HTTP/1.1 (RFC 9113 §8.2.3)* [GitHub pull request #901]. Cloudflare Pingora. https://github.com/cloudflare/pingora/pull/901
