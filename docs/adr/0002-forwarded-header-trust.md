...
RFC 9110 `Via` has a different role. It records the protocol and recipient chain across intermediaries. A proxy must add an appropriate `Via` entry to each message it forwards, while an HTTP-to-HTTP gateway must at least add it to each inbound request. It is therefore preserved as protocol trace while remaining outside the trusted identity model.
