...
Reacquire exact-head formatting, compile/test, clippy, rustdoc, 100% owned-production coverage, dedicated routed traffic/load/OCI, security/supply-chain, and review/thread evidence after the latest descendant movement. The current stack already has source contracts for streamed body overflow, in-flight saturation/recovery, refusal, connected read stall, causally pinned post-header truncation, graceful drain, least-privilege one-binary-per-image OCI startup, and Rust-only routed four-VU/400-request local p95<20 ms regression; source presence is not GREEN.
