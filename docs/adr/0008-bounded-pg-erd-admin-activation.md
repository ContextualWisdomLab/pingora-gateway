...
A fourth constraint emerged from effective socket authority rather than field equality. A traffic listener such as `0.0.0.0:8080` and a metrics listener such as `127.0.0.1:8080` are different `SocketAddr` values but can compete for one port authority at bind time. An IPv6 wildcard can also consume the IPv4 port on dual-stack platforms when `IPV6_V6ONLY` is disabled, and an IPv4-mapped IPv6 address names the same concrete IPv4 authority. Those aliases must fail before Pingora receives network authority. The same review found that generic `GatewayConfig` still admitted port-zero traffic, metrics and upstream sockets even though the bounded migration profile rejected them: listener port zero delegates authority to an OS-selected ephemeral port, metrics port zero makes the declared endpoint indeterminate, and upstream port zero is not a concrete connectable authority.
