...
A fourth constraint emerged from effective socket authority rather than field equality. A traffic listener such as `0.0.0.0:8080` and a metrics listener such as `127.0.0.1:8080` are different `SocketAddr` values but can compete for one port authority at bind time. An IPv6 wildcard can also consume the IPv4 port on dual-stack platforms when `IPV6_V6ONLY` is disabled. IPv4-mapped IPv6 addresses represent IPv4 nodes in IPv6 form, so exact mapped/native aliases overlap; the same fail-closed rule must also cover a native IPv4 wildcard paired with a mapped concrete IPv4 address and a mapped `0.0.0.0` paired with native IPv4 authority. Those cases must fail before Pingora receives network authority. The same review found that generic `GatewayConfig` still admitted port-zero traffic, metrics and upstream sockets even though the bounded migration profile rejected them: listener port zero delegates authority to an OS-selected ephemeral port, metrics port zero makes the declared endpoint indeterminate, and upstream port zero is not a concrete connectable authority.
...
Traffic and metrics addresses must not overlap one effective socket authority. The shared `edge_contract::socket_authorities_overlap` invariant rejects equal addresses, same-port same-family wildcard aliases, exact IPv4-mapped/native IPv4 aliases, native or mapped IPv4 wildcard combinations, and same-port IPv6-wildcard/IPv4 combinations whose dual-stack bind behavior is platform-dependent. Distinct concrete non-aliased IP addresses remain configurable on the same non-zero port. Generic `GatewayConfig` and the migration-specific Admin Config consume this same overlap invariant, while `UpstreamConfig` rejects transport port zero at the shared value boundary. The migration profile performs its characterized zero-port check before shared upstream validation so its established `ZeroTransportAuthorityPort` contract remains stable.
...
The generic `cwl-pingora-gateway` binary, `GatewayConfig` v1 public shape, and one-upstream semantics stay unchanged. Its network-authority validation is tightened to require non-zero traffic/metrics/upstream ports and the shared effective-authority overlap invariant because the same admission defects existed independently of pg-erd routing. Process identity still makes the intended migration deployment contract explicit rather than silently changing generic routing semantics.
...
The routed-load descendant also received a test-first evidence repair. RED `77435efcc377bcaf3ac831421c451a99d1f9f7ce` proves that an aggregate-only 400-request pg-erd p95 gate cannot attribute a tail regression to the characterized `backend` or `frontend` route. GREEN `ec4e171387b5001887dbbc7fecba741b240df453` tags each measured request by route and requires aggregate, backend, and frontend `http_req_duration` p95 each below 20 ms without reducing the four-VU/400-request sample, status/body checks, or zero-failure acceptance. This is local performance-evidence attribution only; it does not change routing or production runtime semantics.
...
The current pg-erd source entryPoint is HTTP. HTTPS/TLS listener activation, HTTP/2 or HTTP/3, WebSocket/Extended CONNECT, explicit TCP reset, broader streaming/upgraded failure, slow-drip/whole-response lifetime, retries beyond the generic one-attempt invariant, representative TLS/network/multi-hop/origin-capacity performance, shadow/canary, cutover and legacy removal remain separate evidence-backed decisions. Routed body/backpressure/refusal/read-stall/post-header/drain/OCI/local-load source contracts now exist on descendants, including aggregate and route-local backend/frontend p95 gates for the four-VU/400-request regression, but none transfers promotion credit until the final unchanged exact head executes terminally.
...
Reacquire exact-head formatting, compile/test, clippy, rustdoc, 100% owned-production coverage, dedicated routed traffic/load/OCI, security/supply-chain, and review/thread evidence after the latest descendant movement. The current stack already has source contracts for streamed body overflow, in-flight saturation/recovery, refusal, connected read stall, causally pinned post-header truncation, graceful drain, least-privilege one-binary-per-image OCI startup, and Rust-only routed four-VU/400-request aggregate/backend/frontend p95<20 ms regression; source presence is not GREEN.

After those unchanged-head contracts execute terminally, add explicit TCP reset, broader streaming/upgraded failure, slow-drip/whole-response lifetime, stronger payload-free observability assertions, and representative TLS/network/multi-hop/origin-capacity measurements. Only representative deployment measurement on an immutable release may promote the 20 ms objective from a local regression bound to buyer-facing performance evidence. Release/canary additionally requires the unresolved organization dependency/security and Dependency Review gates, immutable artifact provenance/SBOM/reproducibility, rehearsed rollback, and then-live protected review governance.
