...
Add an ignored Linux integration profile plus a manual `self-hosted, linux` workflow. Normal CI compiles the integration target and an executable contract verifies that the workflow remains manual, source-bound, topology-bounded, release-profile-bound, and unable to masquerade as ordinary hosted evidence.

The manual workflow binds checkout to the dispatched exact SHA, requires the repository's reviewed Rust 1.98.0 toolchain and OpenSSL build prerequisites, raises and verifies the file-descriptor soft limit, derives the process-allowed CPU list from `/proc/self/status`, filters `lscpu -p=CPU,NODE,SOCKET` to that same set, and rejects a host below the topology floor before it starts the gateway. The Rust profile independently applies the same process-allowed CPU boundary before it admits or records topology, so workflow preflight and runtime receipt cannot count different CPU sets.
