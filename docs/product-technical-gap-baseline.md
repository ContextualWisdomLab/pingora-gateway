...
This baseline is code-current for the foundation compiler-repair child. Exact source heads, base tips, reviews, workflow/security runs, rulesets, and sibling Context Fabric heads are always re-read live; predecessor evidence never transfers across source, documentation, dependency, base, or governance movement.
...
| Release compiler | Repair candidate | Rust 1.98.1 (2026-09-03) fixes a vtable-generation miscompilation introduced in 1.98.0 that can emit null function pointers and undefined behavior. CI, Supply Chain, crate metadata, coverage policy and the OCI build now select/verify 1.98.1. Docker Official Image does not yet advertise a 1.98.1 Bookworm tag, so the digest-pinned 1.98.0 builder is temporarily bootstrap-only and must be replaced with an immutable official 1.98.1 digest when published. Exact-head hosted evidence has to reacquire before this repair is credited GREEN |
...
| OCI isolation | Implemented candidate hardening | Runtime is uid/gid 65532, read-only-root compatible, capability-free and `no-new-privileges`; builder/runtime base images remain digest-pinned, while the builder selects Rust 1.98.1 before compilation; exact-head OCI/Scorecard evidence must reacquire |
| Dependency policy | Release-blocked | `.github#1605` owns the exact-release Pingora vs patched-`lru` decision. `pingora-gateway#13` separately owns unmaintained `derivative 2.2.0` (`RUSTSEC-2024-0388`, no patched version). `.github#810` independently owns the public non-fork Dependency Review compare-API HTTP 403 availability incident. Known-unsound downgrade, blanket advisory waiver, fail-open 403 handling, or substitute-scanner promotion is prohibited |
| Coverage / public API docs | Gates implemented | Owned production line/region coverage is required at 100%; `#![deny(missing_docs)]` and warning-denied rustdoc cover public APIs. Coverage tooling is explicitly bound to Rust 1.98.1 after the compiler repair. The platform-root peer branch that previously left two uncovered regions now has a focused executable regression; every changed head must satisfy the same gates |
...
2. Prove issue #55 on an unchanged exact head: all release-producing paths compile with Rust 1.98.1, the OCI builder verifies that compiler before `cargo build`, and the temporary bootstrap bridge is replaced with an immutable official 1.98.1 image digest once available.
3. Keep `.github#1605`, `pingora-gateway#13`, and `.github#810` fail-closed until their distinct Pingora policy, derivative, and GitHub dependency-review owner paths are resolved; do not suppress `RUSTSEC-2024-0388` generically.
4. Require the Context Fabric owner to repair EA #39's exact-current-head PostgreSQL invariant failure without weakening semantics, prove the repaired parent terminal GREEN, then restack #40 non-destructively on that exact parent and reacquire all child admission/provenance evidence; do not transfer historical #40 GREEN runs.
5. Add explicit concurrency/backpressure budgets and broader timeout/reset/streaming/network-failure recovery evidence, then benchmark representative consumer traffic before adopting a production 20 ms p95 objective.
...
7. Satisfy then-live protected-branch review/governance without self-approval, bot/model-as-human claims, stale evidence transfer, or routine administrator bypass.
...
9. Only then characterize and migrate the highest-impact consumer whose actual responsibility belongs to the shared edge bounded context.
