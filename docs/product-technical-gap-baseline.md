...
Organization Actions queue/admission ownership remains in `ContextualWisdomLab/.github`, not in leaf source churn. Protected `.github/main` is `a9aeee8fc94ad6002a059b380b268590ce496ef0` at this snapshot. Merged #1870 pinned the central OpenCode review-dispatch workflow off a starved floating `ubuntu-latest` path to explicit `ubuntu-24.04`; that is useful owner-side progress but does not directly repair Pingora repository jobs because the Pingora jobs are already explicitly `ubuntu-24.04`.

Current Pingora #59/#60/#56 jobs still reproduce the pre-checkout class (`steps=[]`, `runner_id=0`) after their latest source movement, while predecessor exact heads eventually acquired runners without no-op leaf commits. The correct classification is intermittent/delayed organization runner assignment, not proof of a leaf source defect. Do not change runner selectors, add no-op commits, weaken gates, or promote predecessor success merely to retrigger execution.

The canonical read-only queue-health PR `.github#1150@bbacf9e81ae954eb8365fbfe1856d8698a768a4a` is stale relative to protected main: fresh compare on this snapshot is 67 ahead / 161 behind with merge base `8c085835fbf77de2321b72fa6b8dd946227e523e`, while its body still describes that old merge base as current. Its dedicated owner must read/adopt the intervening deltas and reconcile non-destructively; Pingora does not modify that repository's source/ref/PR state.
