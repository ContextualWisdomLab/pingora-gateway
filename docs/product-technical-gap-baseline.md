...
A subsequent source review found a distinct lexical fail-open after that repair: the explicit-selector classifier normalized the Cargo command token but inspected the following selector token before shell quote/escape reduction. Valid shell forms such as `cargo '+1.98.0' build`, `cargo "+1.98.0" build`, `cargo \+1.98.0 build`, and `cargo "+"1.98.0 build` can therefore deliver `+1.98.0` to rustup's Cargo proxy while escaping the predecessor check. RED `4a152bb2359dc45792118451ab87b70cfb80f8e1` adds the unchanged regression. Current GREEN `d014fa3...` adds `tests/toolchain_shell_word_contract.rs`, conservatively reduces shell quotes/escapes for security-sensitive word comparison, scans semantic YAML `run` scripts, rejects quoted/escaped `+<toolchain>` forms, and still accepts ordinary `cargo build --release --locked`. No production gateway or workflow source changed. Current exact CI `33937214152` and Supply Chain `33937214171` supersede all predecessor execution; CI jobs remain pre-checkout queued with no steps or runner identity, so exact-head GREEN is not yet credited.
...
Pingora predecessor heads have already shown that `materialized/no runner → delayed assignment/exact checkout → terminal semantic result` can occur without leaf no-op churn. The latest #56 RED wave also shows `materialized/no runner → cancelled before execution` when a valid successor supersedes it. Current #59/#60 and the newer #56/#54/#61 heads remain incomplete until their exact jobs acquire runners and reach terminal conclusions. These states must be distinguished from leaf source defects. Do not change runner selectors, add no-op commits, weaken gates, or promote predecessor success merely to retrigger execution.
