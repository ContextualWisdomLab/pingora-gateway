...
Draft #56 current exact head is `3c669a1d7c2f8d230bb38a96ac8fe0b75203f250` on foundation #1, fresh compare **ahead 19 / behind 0** with the foundation as exact merge base. It moves release-producing paths to Rust 1.98.1, the 2026-09-03 repair release for the Rust 1.98.0 vtable-generation miscompilation. Predecessor `955b3e98e1f1bd945a05b8a7cdd5f16e75c99c77` proved exact checkout, Rust 1.98.1 selection, OCI and load execution; its remaining test failure was rustfmt and was repaired forward.

The compiler contract now carries two separately verified fail-closed repairs. First, predecessor `68fffc8a66a11b4657c259739ae1c3984f5818d7` replaced literal `cargo +` matching with line-continuation normalization and token-based `cargo` followed by non-empty `+<toolchain>` detection, covering multiple spaces, tabs, continuation and explicit `/cargo` paths. Fresh inspection then found a different discovery gap: host release-producing jobs were selected with `match_indices("cargo ")`, so valid shell whitespace such as `cargo\tbuild --release --locked` could cause the entire job to escape the per-job Rust 1.98.1 install/select/verify contract. RED `5ddcf4965aa2da02f572a1ef06fc1e4059c35db4` adds an unchanged synthetic regression; GREEN `3c669a1d...` discovers Cargo commands by normalized shell tokens and checks compiler ordering against the same normalized job representation. No workflow/compiler selector or production gateway behavior changed. Current exact CI `33935849215` and Supply Chain `33935849357` supersede all predecessor execution and must still produce terminal exact-head GREEN before compiler promotion.

Draft #54 current head `c62c7d998966edd9321087e982331aa4f0caf7e3` is a non-force child of current #56. Fresh compare against #56 is **ahead 22 / behind 0** with `3c669a1d...` as exact merge base and an effective child delta of exactly four derivative-advisory evidence files. The current compiler repair was adopted in forward commit `abee3e5bcd097176e33c678b88e54ca632c68ea3`, then ordinary two-parent merge `c62c7d...` recorded current #56 without rewriting history. Current CI `33935893555` and Supply Chain `33935893588` supersede earlier child evidence.
...
2. Obtain exact-head GREEN for Rust 1.98.1 #56 after the compiler-authority discovery repair; only then execute #54's intentional `derivative` RED and drive #889 to a maintainer-integrated immutable supplier repair before a downstream lockfile GREEN.
