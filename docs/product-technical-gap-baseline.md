...
A subsequent source review found a distinct shell-word fail-open: quoted or escaped words could hide either the Cargo command or rustup's `+<toolchain>` selector. The initial narrow RED `4a152bb2359dc45792118451ab87b70cfb80f8e1` and intermediate GREEN `d014fa3abc3d69c759f8de73ecaf7112c6465f3d` proved selector quoting in a separate workflow-only oracle, but self-review showed that parallel helper did not repair the canonical `tests/toolchain_contract.rs` path used for both workflows and Dockerfile authority. Canonical RED `aeb0879fc10fabd24476ce3be248c0aff4c21b74` therefore adds unchanged quoted/escaped selector and quoted Cargo-command regressions directly to the existing contract. Canonical GREEN `f9e910e3f5db4cddcbc80a160538b1319f23a093` adds one conservative shell-word reduction used by both Cargo-command discovery and selector detection. It rejects forms including `cargo '+1.98.0' build`, `cargo "+1.98.0" build`, `cargo \+1.98.0 build`, `cargo "+"1.98.0 build`, `car"go" +1.98.0 build`, and `car\go +1.98.0 build`, while ordinary `cargo build --release --locked` remains accepted. Current cleanup `e942563...` removes the redundant separate test file after every valid regression was inherited by the canonical contract, restoring the intended 11-file effective #56 scope without discarding test/evidence delta.

Current exact #56 workflows are CI `33937726206` and Supply Chain `33937726210`; CI jobs remain pre-checkout queued with no steps or runner identity, so exact-head GREEN is not yet credited.

Draft #54 current head `ed54e688068b5cde2cf7dc85723f1d2294f8eeda` is a non-force child of current #56. Fresh compare against #56 is **ahead 26 / behind 0** with `e942563...` as exact merge base and an effective child delta of exactly four derivative-advisory evidence files. The child adopted the current canonical compiler-contract blob and removed the redundant inherited test file in its existing tree; ordinary two-parent merge `ed54e688...` records current #56 without history rewrite. Current exact workflows are CI `33937748964` and Supply Chain `33937749003`, with CI jobs still pre-checkout queued.
