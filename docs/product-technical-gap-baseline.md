...
CodeQL required run `35426685246` is now terminal **FAILURE in deliberate handoff semantics**, not a vulnerability verdict. Language detection `105853767802` succeeded; the Python `105895014864`, JavaScript/TypeScript `105895014900`, and Actions `105895014960` compatibility shards failed only at `Release runner or enforce current-head CodeQL verdict` after reading a pending dispatch verdict. Coordinator `105921625299` subsequently acquired a runner and completed **SUCCESS**, dispatching the exact current-head scan. The resulting central `.github@e6334e229581a918e2f22de18733b76fa65d7e71` repository-dispatch run `35471064725` targets `pingora-gateway#1@38db194...` against base `f8b4c99...` and is currently queued. No current-head SARIF verdict has executed yet. The local terminal failure is therefore a handoff receipt, not gateway source/security RED, and does not justify source churn or a blind rerun. Draft-triggered CI `35426685241` and Supply `35426685243` are skipped and are not GREEN.
