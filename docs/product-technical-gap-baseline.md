...
Fresh self-review then found a compiler-authority bypass outside rustup's selector syntax. Cargo recognizes `RUSTC` as the compiler executable and the configuration environment variable `CARGO_BUILD_RUSTC` as the `build.rustc` executable override. An otherwise valid workflow could therefore install, select and verify default Rust 1.98.1, then execute `RUSTC=/tmp/rustc-1.98.0 cargo build`; the predecessor oracle did not reject that path. RED `86c1d1d958811e72f57c78e6dc7662dc876c3bf3` adds the unchanged synthetic bypass to the canonical contract. GREEN candidate `d7267dfad285e37d7704ab7809620ae5c5061a07` rejects direct `RUSTC=` and `CARGO_BUILD_RUSTC=` authorities in the same workflow/Docker compiler path. This is a contract-only repair; production and workflow source are unchanged. Cargo's primary environment-variable reference documents `CARGO_BUILD_RUSTC` as the `build.rustc` executable selector: https://doc.rust-lang.org/cargo/reference/environment-variables.html.

Current exact #56 workflows are CI `33938618508` and Supply Chain `33938618581`; CI jobs `test 101231406271`, `oci-runtime 101231406400`, and `load-contract 101231406535` are pre-checkout queued with no runner identity, so exact-head GREEN is not yet credited. A fresh exact-range review is required for `5a62e2fa...d7267dfa`; predecessor review credit does not transfer.

Draft #54 current head `02cb82004ce34ce213fd4528b99420461937a8e0` is a non-force child of current #56. Fresh compare against #56 is **ahead 28 / behind 0** with `d7267dfa...` as exact merge base and an effective child delta of exactly four derivative-advisory evidence files. Forward adoption commit `b391c37854b81a6b288d27c441f720c1eeed8b42` carries the parent compiler-environment guard; ordinary two-parent merge `02cb82004ce34ce213fd4528b99420461937a8e0` records current #56 without history rewrite. Current exact workflows are CI `33938649533` and Supply Chain `33938649531`, both queued. Fresh independent exact-range review of `d7267dfa...02cb820` reported no actionable findings and independently confirmed the exact merge base and four-file child scope; hosted execution remains separately required.
