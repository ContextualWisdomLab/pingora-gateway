...
| DDD ownership | Implemented | Edge admission invariants live in `edge_contract`; consumer-derived transport-neutral route characterization lives in `edge_routing`; edge response-header characterization lives separately in `http_policy`; `migration_plan` composes route/header contracts with explicit stable upstream authority; `migration_delivery` binds only those admitted identities to explicit prevalidated Pingora peers. Product auth/business policy, certificate issuance/rotation, Wardnet/EgressWeave decisions, and Keyverse identity remain outside this boundary |
...
| HTTP Policy | Characterized, not activated | Draft parent PR #6 captures `pg-erd-cloud` Traefik response-security fields as a transport-neutral validated policy with ASCII case-insensitive field identity, duplicate rejection, exact values and CR/LF rejection. It is not wired into Pingora response callbacks yet |
| Migration Plan | Characterized, not activated | Draft parent PR #7 composes the pg-erd route and response-policy contracts with an explicit normalized upstream-authority set. Only `backend` and `frontend` are admitted for the captured consumer; unknown route targets fail closed. This does not widen `GatewayConfig` v1 or activate multi-upstream Pingora transport |
| Migration Delivery | Bound, not traffic-activated | This child requires exactly one explicit validated `UpstreamConfig` / Pingora peer for every upstream identity admitted by an `EdgeMigrationPlan`, and rejects missing, duplicate, or undeclared concrete authority. Route selection remains transport-neutral and `GatewayProxy` is unchanged, so no multi-route request serving or cutover is claimed |
...
| Coverage / public API docs | Gates implemented | Owned production line/region coverage is required at 100%; `#![deny(missing_docs)]` and warning-denied rustdoc cover public APIs. New `edge_routing`, `http_policy`, `migration_plan`, and `migration_delivery` production statements/branches are subject to the same exact-head gate |
...
3. Keep PR #5, PR #6, PR #7, and this Migration Delivery child pre-traffic until their exact-head quality/security evidence is terminal. No child evidence repairs a parent release blocker.
4. Next introduce a separate runtime/config transition that consumes `MigrationDeliveryPlan` in the compiled proxy request path and attaches only validated HTTP response policies. Preserve the one-to-one explicit transport-authority invariant; do not admit arbitrary per-request destinations, service discovery by inference, or product authorization/business rules.
5. Prove that runtime transition with compiled-binary pg-erd route/header parity before any shadow/canary work, then add explicit broader timeout/reset/streaming/network-failure recovery and representative routed concurrency/origin-capacity measurements before adopting a production 20 ms p95 objective.
