...
Commit `1b6c5307e6ff7837dd7504ea2cc23936ab44a86c` is the minimal causal repair and changes only rustfmt-required layout in `tests/load/load_origin.rs`; fixture behavior, queue/worker/header bounds, connection mode, sample/VU count, p95 threshold, production gateway code, workflow logic, dependency graph, and supplier pin are unchanged. Fresh exact-head CI `34045381577` and Supply Chain `34045381591` are queued and uncredited at the latest read. The predecessor CodeRabbit review stops at `bad9e0ed...`; it does not transfer current-head review credit to `1b6c5307...`.
