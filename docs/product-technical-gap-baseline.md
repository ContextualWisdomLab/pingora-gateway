...
Compiler repair #56 is current exact `3b0b0f11629b4a4dabde12796ff8a57cfe6deb37`, non-force reconciled onto current foundation `0da81a93f93e869c15bb7d34c55fc87479d16522`. Fresh compare uses foundation as exact merge base, ahead 75 / behind 0, with 15 compiler/bootstrap files in the effective child range. The promoted workflow admission/concurrency/tag-filter contract files are inherited from foundation rather than copied as #56 child delta.
...
Fresh inspection found a distinct computed-alias fail-open: `CARGO="$(command -v cargo)"; "$CARGO" build --release --locked` was not recorded as a persistent Cargo command because the assignment value was not literally `cargo`. Source RED `f5d17516d7c06a8302c579fb08e2ee5c729e7f47` adds three regressions covering assignment-only, `${CARGO:?}` and `export` forms. Its CI `33977577901` and Supply Chain `33977577898` were cancelled before checkout with no runner/steps, so it is source RED rather than hosted RED.

Minimal repair `3b0b0f11629b4a4dabde12796ff8a57cfe6deb37` treats an active `$()` in a persistent assignment value as potential executable authority. That alias is rejected only when later used in command position; command-local assignment prefixes remain non-persistent. This is intentionally fail-closed bounded shell-policy analysis rather than a complete POSIX shell parser.

Current exact #56 CI is `33977617896` and Supply Chain is `33977617965`. At the latest verified read, CI jobs `oci-runtime 101336997646`, `load-contract 101336997894`, and `test 101336997901` are queued pre-checkout with `steps=[]` and `runner_id=0`. Predecessor execution/review results do not transfer; current `3b0b0f11...` is not yet hosted GREEN.
