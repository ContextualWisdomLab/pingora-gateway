...
Both Pingora composition roots call the shared delivery guard after gateway-local health handling but before request admission, route/upstream selection or origin contact. The admitted v1 behavior is HTTP 501 fail-closed, not partial WebSocket support. `tests/protocol_transition_traffic.rs` exercises both compiled listeners with WebSocket-shaped HTTP/1.1 requests, requires exact 501, proves the relevant origins remain untouched, and then requires `/readyz` 200. Its fixture holds traffic/metrics listener reservations concurrently, bounds each response header by a five-second whole-header deadline and 64 KiB, and parses only exact case-sensitive HTTP/1.1 three-digit status tokens so port reuse, slow-drip headers, protocol-case changes or numeric-prefix statuses cannot manufacture GREEN.

The first current-parent hosted execution proved OCI and both generic/routed load paths while failing only at the Rust 1.98.0 formatter gate in the new transport-neutral policy regression. A formatter-only repair changed no gateway semantics. The changed head must still complete its own exact CI/Supply Chain and current-range technical review before #29 can be treated as technically closed. WebSocket enablement requires a separate versioned contract, supplier disposition, handshake/tunnel concurrency, disconnect/backpressure/drain evidence, and distinct HTTP/2 Extended CONNECT handling; removing the v1 rejection alone is not an admissible implementation.
...
| Protocols | HTTP/1.1 migration path exists; current #29 fails closed with 501 on uncharacterized HTTP/1 Upgrade before origin contact | Exact #29 hosted/current-range review closure; downstream TLS/H2, H2→H1 Cookie behavior, versioned WebSocket/Extended CONNECT and explicit H3/QUIC disposition remain separate contracts |
