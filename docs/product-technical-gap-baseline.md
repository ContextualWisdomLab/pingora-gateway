...
Ready supplier-semantics characterization #62 remains exact `d4a54d8d1337797c3763c3d915c674cdf483db1c`. CI `34024689629` executed on that unchanged head. `test 101463462339` is terminal GREEN through exact checkout, Rust 1.98.1, formatting, compile/test, strict Clippy, public rustdoc, pinned coverage tooling, owned-production coverage workload, 100% line/region enforcement, and dependency-lock evidence. `oci-runtime 101463462432` is terminal GREEN under the exact non-root/read-only least-privilege image contract. Supply Chain `34024689818` / `candidate-evidence 101463497940` is also terminal GREEN through committed dependency audit, exact candidate image, SPDX SBOM, image scan, exact-source binding, and evidence upload.
...
Exact job-log RCA now resolves the cause rather than leaving it as a scheduling hypothesis. All five failed measured requests were gateway HTTP 502 responses with Pingora `Upstream ConnectRefused` / `Connection refused (os error 111)` to `127.0.0.1:18081`, all emitted during the first measured second (`2026-09-06T10:20:09Z`); the same unchanged run then served the remaining 395 requests successfully. The inherited load workflow starts `tests/load/upstream_fixture.py` asynchronously, proves only gateway `/livez`, and immediately admits k6 without proving the test-only origin socket has bound. The #62 traffic RED is therefore an origin-fixture startup race, not a supplier-semantics failure, scheduler failure, or p95 regression.

Focused Ready sibling #63 is exact `190cd6aebc0b82b01f04b01ff095a741f831a0c8`, direct child of #56 exact `18fb38b1ba70c4bf222642ef347f3d57a98379a2`, ahead 1 / behind 0 with that exact merge base and only `.github/workflows/ci.yml` changed (+14/-0). Before gateway admission/k6 it now proves the origin process is alive and directly reachable at `127.0.0.1:18081/fixture-ready`, while failing immediately if that process exits. The fixture serves the same deterministic GET payload for any path, so this direct probe proves socket/handler readiness without traversing or warming the measured gateway `/load-contract` route. The 400-request / 4-VU sample, zero-failure requirements, p95 `<20 ms`, runtime, dependency graph, compiler, Pingora source, and routing semantics remain unchanged.

Exact #63 CI `34027297080` and Supply Chain `34027296921` have materialized and remain the current validation wave. They do not establish GREEN until the unchanged exact head actually executes. If #63 restores zero-failure correctness, #62 must adopt the workflow delta by ordinary non-force ancestry and re-prove its supplier characterization on the resulting exact head. If failures remain after direct origin readiness is proven, the next finding must come from the exact runtime/load logs rather than a blind rerun, sample reduction, or threshold relaxation. No predecessor GREEN is transferred.
...
Current exact `d4a54d8d1337797c3763c3d915c674cdf483db1c` is the minimal causal formatting repair. Its supplier-semantics source has proved hosted GREEN in `test 101463462339`, including formatting, compile/test, strict lint, rustdoc, 100% owned-production line/region coverage, and lock verification. `oci-runtime 101463462432` is also GREEN, and Supply Chain `34024689818` / `candidate-evidence 101463497940` is terminal GREEN through dependency audit, image/SBOM scan, and exact-source binding. The sibling load-contract RED described above is a harness startup finding, not a failure of the `PeerOptions` characterization itself. Fresh exact-range CodeRabbit review covers `8027ebfb...d4a54d8d` with no actionable comments; this remains technical review evidence rather than an independent `APPROVED` review.

#62 cannot be promoted by transferring #63 evidence. After #63 GREEN it must adopt the workflow repair non-destructively and execute again on the resulting exact head.
...
Primary execution root: `#62 confirmed origin-startup traffic RED → #63 exact readiness-repair validation → ordinary non-force adoption into #62 → re-prove #62 exact test/load/OCI/Supply Chain`.
