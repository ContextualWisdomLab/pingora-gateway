...
Draft #56 current exact head is `3b70ba734aae5f43a620b32bda5f0b59fe2b602b` on foundation #1, fresh compare **ahead 30 / behind 0** with the foundation as exact merge base. It moves release-producing paths to Rust 1.98.1, the 2026-09-03 repair release for the Rust 1.98.0 vtable-generation miscompilation. Predecessor `955b3e98e1f1bd945a05b8a7cdd5f16e75c99c77` proved exact checkout, Rust 1.98.1 selection, OCI and load execution; its remaining test failure was rustfmt and was repaired forward.

The compiler contract carries one canonical Rust oracle with accumulated causal fail-closed repairs. Predecessor `68fffc8a66a11b4657c259739ae1c3984f5818d7` replaced literal `cargo +` matching with whitespace-aware token detection. RED `5ddcf4965aa2da02f572a1ef06fc1e4059c35db4` then exposed that host release-producing jobs were discovered with `match_indices("cargo ")`, allowing valid `cargo\tbuild` to evade job-local Rust 1.98.1 setup; GREEN `3c669a1d7c2f8d230bb38a96ac8fe0b75203f250` moved command discovery to shell-whitespace tokens. Its RED jobs were cancelled pre-checkout when the successor arrived, so that regression is source-level rather than hosted RED evidence.

Fresh exact-range review of `5a62e2fa...3c669a1d...` found another High: the token parser replaced an unquoted POSIX backslash-newline with a space even though the shell removes the pair without adding a byte. `car\\<newline>go build` therefore executes as `cargo build` but normalized to `car go build`, and `car\\<newline>go +1.98.0 build` could also escape explicit-selector detection. RED `a29b76ba85ba69b5deb4e48e04ac873c0dd2439a` adds unchanged regressions for both paths. GREEN `1948e5b3e35f48bc7bb8f6d57c111d7d23fc019d` parses each workflow through the repository's semantic YAML implementation so block-scalar indentation is removed before shell analysis, removes shell continuations with POSIX semantics, discovers Cargo in the resulting semantic scripts, and keeps raw job inspection for non-run selectors.

A subsequent source review found a distinct shell-word fail-open: quoted or escaped words could hide either the Cargo command or rustup's `+<toolchain>` selector. The initial narrow RED `4a152bb2359dc45792118451ab87b70cfb80f8e1` and intermediate GREEN `d014fa3abc3d69c759f8de73ecaf7112c6465f3d` proved selector quoting in a separate workflow-only oracle, but self-review showed that parallel helper did not repair the canonical `tests/toolchain_contract.rs` path used for both workflows and Dockerfile authority. Canonical RED `aeb0879fc10fabd24476ce3be248c0aff4c21b74` therefore adds unchanged quoted/escaped selector and quoted Cargo-command regressions directly to the existing contract. Canonical GREEN `f9e910e3f5db4cddcbc80a160538b1319f23a093` adds one conservative shell-word reduction used by both Cargo-command discovery and selector detection. It rejects forms including `cargo '+1.98.0' build`, `cargo "+1.98.0" build`, `cargo \+1.98.0 build`, `cargo "+"1.98.0 build`, `car"go" +1.98.0 build`, and `car\go +1.98.0 build`, while ordinary `cargo build --release --locked` remains accepted. Cleanup `e9425635bd13401ad793530782ee68d79eb9ed83` removes the redundant separate test file after every valid regression was inherited by the canonical contract, restoring the intended 11-file effective #56 scope without discarding test/evidence delta.

Fresh self-review then found a compiler-authority bypass outside rustup's selector syntax. Cargo recognizes `RUSTC` as the compiler executable and the configuration environment variable `CARGO_BUILD_RUSTC` as the `build.rustc` executable override. An otherwise valid workflow could install, select and verify default Rust 1.98.1, then execute `RUSTC=/tmp/rustc-1.98.0 cargo build`; the predecessor oracle did not reject that path. RED `86c1d1d958811e72f57c78e6dc7662dc876c3bf3` adds the unchanged synthetic bypass to the canonical contract. GREEN `d7267dfad285e37d7704ab7809620ae5c5061a07` rejects direct `RUSTC=` and `CARGO_BUILD_RUSTC=` shell authorities in the same workflow/Docker compiler path.

That direct-string repair was still fail-open for GitHub Actions YAML environment mappings. Workflow-level `env: { RUSTC: ... }`, job-level `env: CARGO_BUILD_RUSTC: ...`, and step-level `env: RUSTC: ...` affect Cargo while appearing as semantic YAML keys rather than `KEY=value`; standalone `rustc --version` verification remains unaffected. RED `df2cfe12cbed49d556a4f1a87fd37b0f4458538f` adds the unchanged job-level YAML `RUSTC` bypass, which the predecessor oracle accepts. GREEN candidate `3b70ba734aae5f43a620b32bda5f0b59fe2b602b` parses workflow-, job-, and step-level environment mappings semantically and rejects both `RUSTC` and `CARGO_BUILD_RUSTC` for Cargo-executing jobs while preserving the prior shell/selector regressions. No production or workflow source changed. Cargo's primary environment-variable reference documents `CARGO_BUILD_RUSTC` as the `build.rustc` executable selector: https://doc.rust-lang.org/cargo/reference/environment-variables.html.

Current exact #56 workflows are CI `33939316667` and Supply Chain `33939316673`; they were queued/pending on the latest post-push read, so exact-head GREEN is not yet credited. Fresh exact-range review is required for `5a62e2fa...3b70ba73`; predecessor review credit does not transfer.

Draft #54 current head `02a739c0a3fa2772a649ef54bbd3563ac533ec5b` is a non-force child of current #56. Fresh compare against #56 is **ahead 30 / behind 0** with `3b70ba73...` as exact merge base and an effective child delta of exactly four derivative-advisory evidence files. Forward adoption commit `192042da210330030d579dabfbedccc3ae9c9e84` carries the parent YAML compiler-environment guard; ordinary two-parent merge `02a739c0a3fa2772a649ef54bbd3563ac533ec5b` records current #56 without history rewrite. Current exact workflows are CI `33939346427` and Supply Chain `33939346408`, pending on the latest read. The previous clean review of predecessor `d7267dfa...02cb820` is historical after the parent moved; current review is required.
...
Protected public `cloudflare/pingora/main` is `09696b51bc59315353d96686355861604d0bb48c` at this snapshot. Public Cookie candidate #901 remains open/unmerged at `b856ddfc6be15f1727601d2d76cb10d2d72f95f0`; it targets an older `PeerOptions.h2_to_h1_concat_cookies` shape and is not an immutable consumable dependency. A repair must be adapted to current `HttpUpstreamRequestPolicy` with default-on Cookie normalization and a narrow opt-out that preserves unrelated hop-by-hop/Connection protections.

Body-framing candidate #936 remains open/unmerged at `e40ed4cceb0c0ed8c05cc39eb01a8c73dea5497a`, directly on current protected Pingora main. It addresses zero-length chunked writes emitting a premature H1 terminator in both async and cancel-safe write paths. It remains a separate prerequisite from Cookie normalization and is still mutable supplier work until maintainer integration.

Cloudflare issue #889 remains open with no maintainer-integrated immutable repair at this snapshot. Current public source uses `derivative 2.2.0` in `PeerOptions` debug derivation and load-balancing `Backend` identity traits. An acceptable supplier repair must preserve omitted callback/debug fields and keep `Backend::Extensions` excluded from equality/hash/order semantics, remove the dependency declarations, pass upstream tests/Clippy/audit, and become maintainer-integrated immutable supplier truth before the gateway bumps it.

## Actions owner-plane

Organization Actions queue/admission ownership remains in `ContextualWisdomLab/.github`, not in leaf source churn. Protected `.github/main` is `8272e4f95c253ab067592460cc9288581bf3a422` at this snapshot. The newest protected merge #1880 removes repository-side Strix scan retries; this is an owner-side review/admission change, not Pingora leaf GREEN evidence. Earlier #1878/#1877 queue and review-control repairs remain part of the intervening owner history. Current Pingora waves still require their own exact runner assignment and terminal semantic evidence.

Pingora predecessor heads have already shown that `materialized/no runner → delayed assignment/exact checkout → terminal semantic result` can occur without leaf no-op churn. Other current heads have also shown `materialized/no runner → cancelled before execution` when a valid successor supersedes them. Current #59/#60 and the newer #56/#54/#61 heads remain incomplete until their exact jobs acquire runners and reach terminal conclusions. These states must be distinguished from leaf source defects. Do not change runner selectors, add no-op commits, weaken gates, or promote predecessor success merely to retrigger execution.

The canonical read-only queue-health PR `.github#1150@bbacf9e81ae954eb8365fbfe1856d8698a768a4a` is stale relative to protected main: fresh compare against `main@8272e4f95c253ab067592460cc9288581bf3a422` is **67 ahead / 171 behind** with merge base `8c085835fbf77de2321b72fa6b8dd946227e523e`, while its body still describes that old merge base as current and says the branch is no longer behind. Its dedicated owner must read/adopt the intervening deltas and reconcile non-destructively; Pingora does not modify that repository's source/ref/PR state.
...
2. Obtain current exact review and hosted GREEN for Rust 1.98.1 #56 after the semantic-YAML/POSIX-continuation, shell-word, direct Cargo compiler-environment, and YAML env-scope authority repairs; only then execute #54's intentional `derivative` RED and drive #889 to a maintainer-integrated immutable supplier repair before a downstream lockfile GREEN.
...
4. Keep organization queue/admission RCA on the `.github` owner path. Pingora leaf heads wait/refetch without no-op churn when the only evidence is pre-checkout runner starvation or valid successor cancellation.
