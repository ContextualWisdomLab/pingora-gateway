...
Draft #56 is now exact `8f03a6ca1f09c6fe0f4edf47438f17e83c801d0b` on foundation #1, fresh compare **ahead 34 / behind 0** with 11 effective files. Fresh review of predecessor `50b1f8c4b645622f61dfa25f74ac2bb82968837e` closed the prior shell-normalization findings but found one remaining High compiler-authority gap: YAML workflow/job/step `env` rejected `RUSTC` and `CARGO_BUILD_RUSTC` but not `RUSTUP_TOOLCHAIN`, allowing a later Cargo step to rebind the verified 1.98.1 default to 1.98.0.

Current `8f03a6ca...` is the minimal causal repair. The semantic YAML environment guard now rejects `RUSTUP_TOOLCHAIN` together with `RUSTC` and `CARGO_BUILD_RUSTC`, and a step-level `RUSTUP_TOOLCHAIN: 1.98.0` regression was added. Per-job Rust 1.98.1 install/select/verify ordering, shell-normalized authority checks, post-verification override rejection, Docker compiler authority, Cargo `+<toolchain>` detection, and the `rust-version = "1.98.1"` requirement remain intact. Current exact runs CI `33955055687` and Supply Chain `33955055597` are queued/pending; current-head compiler/bootstrap GREEN is absent and predecessor terminal results do not transfer. A fresh exact-range review of `5a62e2fa...8f03a6ca` has been requested.

Draft #54 was non-force restacked onto current #56 with ordinary two-parent commit `991a0001320b2d5f07d9fb699d5181717c871887`. Its first parent is old child `3d65b786ec19f8c139b575117115767de3ac90be`, second parent is current #56 `8f03a6ca...`, and the tree adopts the repaired `tests/toolchain_contract.rs` blob while preserving exactly four effective child files: `CHANGELOG.md`, `TEST_STRATEGY.md`, `docs/doctoring/TRACEABILITY.md`, and `tests/supply_chain_policy.rs`. Fresh compare is **ahead 33 / behind 0** with current #56 as exact merge base. Current runs CI `33955141682` and Supply Chain `33955141669` are queued/pending, so the intended `derivative` advisory failure is not promoted until #56 independently proves compiler/bootstrap GREEN.
