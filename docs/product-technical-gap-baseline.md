...
| DDD ownership | Implemented | Edge admission invariants live in `edge_contract`; consumer-derived transport-neutral route characterization lives in `edge_routing`; edge response-header characterization lives separately in `http_policy`; migration-level composition of route/header contracts with explicit upstream authority lives in `migration_plan`; Pingora types and trust-bundle loading stay in delivery/application modules. Product auth/business policy, certificate issuance/rotation, Wardnet/EgressWeave decisions, and Keyverse identity remain outside this boundary |
...
| Migration Plan | Characterized, not activated | This child composes the pg-erd route and response-policy contracts with an explicit normalized upstream-authority set. Only `backend` and `frontend` are admitted for the captured consumer; unknown route targets fail closed. This does not widen `GatewayConfig` v1 or activate multi-upstream Pingora transport |
...
| Coverage / public API docs | Gates implemented | Owned production line/region coverage is required at 100%; `#![deny(missing_docs)]` and warning-denied rustdoc cover public APIs. New `edge_routing`, `http_policy`, and `migration_plan` production statements/branches are subject to the same exact-head gate |
...
3. Keep PR #5, PR #6, and this Migration Plan child characterization-only until their exact-head quality/security evidence is terminal. No child evidence repairs a parent release blocker.
4. After those contracts are stable, introduce an explicit versioned runtime/config transition that consumes the validated `EdgeMigrationPlan`, binds only admitted route identities to prevalidated Pingora peers, and attaches only validated HTTP response policies. Do not admit arbitrary per-request destinations or product authorization/business rules.
