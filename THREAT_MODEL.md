...
| Recursive/self-targeted upstream authority | Startup validation rejects any upstream whose effective socket authority overlaps the process traffic or metrics listener, including wildcard/native/IPv4-mapped aliases | Host-network topology outside the configured socket authorities still belongs to deployment/network owners |
...
| Unsafe rollout | Pull-request governance, process health, generic graceful-drain test, explicit immutable-artifact/rollback requirements | Dedicated routed drain, shadow/canary, protected cutover and rehearsed rollback against a released digest remain pending |
