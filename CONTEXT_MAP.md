...
| Migration Plan | Supporting composition | Explicit migration upstream-authority set plus validated Edge Routing and HTTP Policy | Transport-neutral prerequisite for later migration delivery; validates identity authority but does not grant network authority |
...
The active v1 shared kernel is the public versioned configuration vocabulary. `edge_routing`, `http_policy`, and `migration_plan` are executable migration characterizations/composition until a later versioned runtime/config transition explicitly admits multi-route serving. There is no shared Rust domain model with consumer products, Context Graph, Enterprise Architecture, Wardnet/EgressWeave, or Keyverse.
