...
The active v1 `GatewayConfig` aggregate admits one listener, one upstream authority, a positive request-body limit, a positive service-worker count, an explicit TLS identity when TLS is enabled, and positive upstream I/O budgets. Worker topology is an operator runtime-capacity input: omitted unreleased v1 configurations preserve one worker per Pingora service, while an explicit value must be validated and propagated without inferring host CPU count. Characterized route tables or response-header policies do not become active merely because their transport-neutral modules exist. Any activation requires an explicit versioned configuration/runtime transition and executable consumer parity evidence.
